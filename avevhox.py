#About:  Avevhox is a portmanteau of the latin words for Voice + Eliminator.  It's an app for eliminating the sounds of human voices from .wav files.
...
ALLOWED_EXTENSIONS = set(['txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', 'wav', 'ico'])
...
app.config.update(dict(
    DATABASE=os.path.join(app.root_path, '/tmp/avevhox.db'),  #MacHD/tmp/avevhox.db! 
    DEBUG=True,
...
def add_entry():
    ...
    db.commit()
    flash('Thank You!')
    return render_template('thanks.html', next=next)
    return render_template('layout.html', next=next)
    #return 'Thank You!' ##THIS WORKS!
    #return redirect(url_for('layout.html', region=None, ip=None))
